...
static struct argp_option cmd_options[] =
    { { "port", 'p', "port", 0, "Port number 0-255 (default 0)" },
        ...
        { "rate", 'i',
            "Mbps", 0, "Mbits/s transmission rate" },
        { "duplicate", 'd', 0, 0,
...
//		fifo->fifo_array[i].seq_nr = 0;
    ...
    for (k = 0; k < OVERLAY_NUM; ++k) {
      ...
      fifo->fifo_array[i].pb_overlay[k].data_blocks = (unsigned char **) malloc(
...
//							MAX_PACKETS_PER_BLOCK * sizeof(unsigned char *));
      ...
      for (j = 0; j < MAX_PACKETS_PER_BLOCK; ++j) {
        ...
        struct pkt_struct_t * ps = (struct pkt_struct_t *) fifo->fifo_array[i]
            .pb_overlay[k].pbl_placeholder[j].data;
        ...
        ps->RadiotapHeader.sRadiotapHeader.radiotap_header.it_present = (1
            << IEEE80211_RADIOTAP_TX_FLAGS) | (1 << IEEE80211_RADIOTAP_RATE);
        ps->RadiotapHeader.sRadiotapHeader.DataRate = inj_rate * 2;
        ps->RadiotapHeader.sRadiotapHeader.TxFlags =
            IEEE80211_RADIOTAP_F_TX_NOACK | IEEE80211_RADIOTAP_F_TX_SEQ;
...
//                memcpy(ps->IeeeHeader, u8aIeeeHeader, sizeof(u8aIeeeHeader));
        ...
        ps->IeeeHeader.sIeeeHeader.seq_ctrl =
            (IEEE80211_SCTL_SEQ & (0x861 << 4)) | (IEEE80211_SCTL_FRAG & 0x0);
...
//        printf("pb_process_block TX pkt blknum %d\n", blknum);
    pb_transmit_packet(
        ...
        curr_fifo->pb_overlay[curr_fifo->curr_proc_overlay].pbl_data[curr_fifo
            ->pb_overlay[curr_fifo->curr_proc_overlay].tx_pb_data++].data,
        blknum, 0);
  }
#endif
  ...
  for (i = 0;
      ...
      ++i) {
    packet_buffer_t *pb = &curr_fifo->pb_overlay[curr_fifo->curr_proc_overlay]
        .pbl_data[curr_fifo->pb_overlay[curr_fifo->curr_proc_overlay]
        .data_part_ratio + i];
  ...
  if ((signum == SIGALRM) || (signum == SIGWINCH)) {
    ...
    fprintf(
        ...
        ((float) bytes_sent * 8 / 1000)
            / ((float) now_time.tv_sec + (float) now_time.tv_nsec / 10e9
                - (float) last_time.tv_sec - (float) last_time.tv_nsec / 10e9));
    last_time = now_time;
    bytes_sent = 0;
  } else {
...
//                	printf("TX blknum %d\n", blknum);
          pb_transmit_packet(
              ...
              pfifo->fifo_array[i].pb_overlay[cpo_val].pbl_data[pfifo
                  ->fifo_array[i].pb_overlay[cpo_val].tx_pb_data++].data,
              blknum, 0);
        }
#endif
        sched_yield();
...
{
  int64_t packet_length = (int64_t)(PACKET_OVERHEAD_LEN * curr_byte_rate * (fec_packets + data_packets)) /
      ((int64_t)(inj_rate_max * data_packets) - (int64_t)(curr_byte_rate * (fec_packets + data_packets)));
                                       ...
                                       int32_t packet_payload_length)
{
  int16_t fec_packets = (int64_t)(inj_rate_max * data_packets) /
      (int64_t)(curr_byte_rate * (PACKET_OVERHEAD_LEN / packet_payload_length  + 1)) - (int64_t)data_packets;
//  if((fec_packets % 10) >=5 )
//    fec_packets += 10;
//  fec_packets /= 10;
  return fec_packets;
...
static uint16_t calc_fec_params(int16_t * data_packets_out, int16_t * fec_packets_out,
                            uint32_t inj_rate_max, uint64_t bytes_received) {
  static uint16_t prev_data_packets = 1;
  static uint16_t prev_prev_data_packets = 1;
//  prev_data_packets = *data_packets_out;
...
//packet_payload_length = (PACKET_OVERHEAD * curr_byte_rate * (fec_packets + data_packets)) / ((param_injection_rate * data_packets) - (curr_byte_rate * (fec_packets + data_packets)))
  ...
  fprintf(stderr, "\n>Fec calc, packet_payload_length = %d, bytes_received = %ld, prev_data_packets = %d\n",
          packet_payload_length, bytes_received, prev_data_packets);

//  while(1){
  for(uint8_t i=0; i<3; ++i){
    fprintf(stderr, ">>iter=%d\n", i);
    time_delta = (NSEC_IN_SEC * packet_payload_length * prev_data_packets) / curr_byte_rate;
...
//        - (int64_t) (1 * NSEC_IN_SEC);
    int64_t fec_part_ratio = (int64_t) (NSEC_IN_SEC * inj_rate_max) /
        (int64_t)((curr_byte_rate * PACKET_OVERHEAD_LEN) / packet_payload_length + curr_byte_rate) - (int64_t) (1 * NSEC_IN_SEC);
    fprintf(stderr, ">>>prev_data_packets=%d, packet_payload_length = %d, time_delta=%ld ns, bytes_received=%ld, "
            "ratio=%f, param_injection_rate=%d, data rate=%ld\n",
            prev_data_packets, packet_payload_length, time_delta, bytes_received,
...
//                     inj_rate_max, curr_byte_rate);
//        }
        packet_payload_length = calc_packet_length(curr_byte_rate, 1, 1, inj_rate_max) + 1;
      ...
      if (fec_part > (data_packets * NSEC_IN_SEC)) {
        data_packets = 1;
        fec_packets = 1;
...
//                     - data_packets;
        fec_packets = calc_fec_packets(curr_byte_rate, data_packets, inj_rate_max, packet_payload_length);

      } else if (fec_part == (1 * NSEC_IN_SEC)) {
        fec_packets = 1;
      } else {

        scale = ((uint64_t)MAX_PACKETS_PER_BLOCK * NSEC_IN_SEC)
...
//              / NSEC_IN_SEC - data_packets;
      ...
      if (packet_payload_length < MAX_USER_PACKET_LENGTH_WO_LEN) {
        //Trying to make fec packets = data packets = prev_data_packets by raising packet payload length
//        packet_payload_length = ((int64_t)PACKET_OVERHEAD_LEN * 2 * (int64_t)curr_byte_rate) / ((int64_t)inj_rate_max - 2 * (int64_t)curr_byte_rate) + 1;
        packet_payload_length = calc_packet_length(curr_byte_rate, 1, 1, inj_rate_max);
        fprintf(stderr, "+++New packet length %d\n", packet_payload_length);
        if (packet_payload_length > MAX_USER_PACKET_LENGTH_WO_LEN){
          packet_payload_length = MAX_USER_PACKET_LENGTH_WO_LEN;
        }
      ...
      data_packets = MAX_PACKETS_PER_BLOCK;
//      fec_packets = ((inj_rate_max * time_delta * data_packets)
//          / (NSEC_IN_SEC * prev_data_packets * (PACKET_OVERHEAD_LEN  + packet_payload_length)))
//           - data_packets;
      fec_packets = calc_fec_packets(curr_byte_rate, data_packets, inj_rate_max, packet_payload_length);
...
//      / (time_delta * data_packets / prev_data_packets);
  uint64_t new_byte_rate = ((fec_packets + data_packets) * curr_byte_rate * (PACKET_OVERHEAD_LEN  + packet_payload_length)) /
      (packet_payload_length * data_packets);
  fprintf(stderr, "New byte rate: %ld, prev_data_packets=%d, bytes_received=%ld, fec_packets=%d, data_packets=%d, packet_payload_length=%d\n",
      new_byte_rate, prev_data_packets, bytes_received, fec_packets,
      data_packets, packet_payload_length);
  prev_prev_data_packets = prev_data_packets;
  prev_data_packets = data_packets;
  *data_packets_out = data_packets;
  *fec_packets_out = fec_packets;
  return packet_payload_length;
}
  ...
  switch (key) {
    ...
    case ARGP_KEY_END:
      ...
      if (opts->data_packets_per_block > MAX_DATA_OR_FEC_PACKETS_PER_BLOCK
          || opts->fec_packets_per_block > MAX_DATA_OR_FEC_PACKETS_PER_BLOCK) {
        fprintf(
            stderr,
            "Data and FEC packets per block are limited to %d (you requested %d data, %d FEC)\n",
            MAX_DATA_OR_FEC_PACKETS_PER_BLOCK,
            opts->data_packets_per_block, opts->fec_packets_per_block);
        argp_usage(state);
        break;
      }
...
//setting the timer for cyclic alarm each 2 sec
  const struct itimerspec tp_int = { .it_interval = { 2, 0 }, .it_value =
      { 2, 0 } };
...
//                printf("j=%d, nfds=%d, nfds_mask=%x\n", j, nfds, nfds_mask);
      ...
      packet_buffer_t *pb =
          &fifo.fifo_array[i].pb_overlay[used_rx_ovl].pbl_data[fifo.fifo_array[i]
              .pb_overlay[used_rx_ovl].curr_pb_data];
...
//			}
      ...
      debug_print(
          ...
          fifo.fifo_array[i].pb_overlay[used_rx_ovl].curr_pb_data, used_rx_ovl,
          i);
...
//                ph->data_length = pb->len - sizeof(payload_header_t); //write the length into the packet. this is needed since with fec we cannot use the wifi packet lentgh anymore. We could also set the user payload to a fixed size but this would introduce additional latency since tx would need to wait until that amount of data has been received
        ...
        pcnt++;
                fprintf(stderr, "Bytes received combined %d, packet_length %d, min_packet_length %d, num_data_blocks %d, num_fecs_blocks %d, pcnt %d\n",
                		bytes_received, run_packet_length, run_min_packet_length, ps->payload_hdr.num_data_blocks, ps->payload_hdr.num_fecs_blocks, pcnt);
        ...
        if (fifo.fifo_array[i].pb_overlay[used_rx_ovl].curr_pb_data
          ...
          uint64_t prev_data_packets =
              fifo.fifo_array[i].pb_overlay[used_rx_ovl].data_part_ratio;
          uint64_t prev_fec_packets = fifo.fifo_array[i].pb_overlay[used_rx_ovl]
              .fec_part_ratio;
          fifo.fifo_array[i].pb_overlay[used_rx_ovl].curr_pb_data++;
          ...
          fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length = calc_fec_params(
              &fifo.fifo_array[i].pb_overlay[used_rx_ovl].data_part_ratio,
              &fifo.fifo_array[i].pb_overlay[used_rx_ovl].fec_part_ratio,
              run_opts.injection_rate, bytes_received) + sizeof(ps->payload.sPayload.len);
          fprintf(stderr, "After calc_fec, plen %ld\n", fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length);
          //for now
...
//            debug_print(
          ...
          if (fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length
            ...
            debug_print(
                ...
                last_payload_len);
            fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length =
                last_payload_len + sizeof(ps->payload.sPayload.len);
          ...
          if (fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length
              > MAX_USER_PACKET_LENGTH) {
            printf(
                "Wrong packet length %d, rate %d, FEC %d, data %d\n",
                fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length,
                curr_byte_rate,
                fifo.fifo_array[i].pb_overlay[used_rx_ovl].fec_part_ratio,
                fifo.fifo_array[i].pb_overlay[used_rx_ovl].data_part_ratio);
...
//                    printf("Final packet_payload_length %d\n", fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length);
          ...
          run_packet_length = fifo.fifo_array[i].pb_overlay[used_rx_ovl].packet_payload_length - sizeof(ps->payload.sPayload.len);
//          debug_print(
          fprintf(stderr,
