...
{
	printf(
	    "tx_alternate (c)2015 befinitiv. Based on packetspammer by Andy Green. Dirty mods by Rodizio.  Licensed under GPL2\n"
	    "\n"
	    "Modified version to send out packets in an alternating pattern on two interfaces\n"
	    "\n"
	    "Usage: tx_alternate [options]\n"
	    "\n"
	    "Options:\n"
	    ...
	    "\n"
	    "Example:\n"
	    "  cat /dev/zero | tx -b 8 -r 4 -f 1024 wlan0 wlan1 (reads zeros from stdin and sends them out alternating on wlan0 and wlan1)\n"
...
void pb_transmit_packet(pcap_t *ppcap1, pcap_t *ppcap2, int seq_nr, uint8_t *packet_transmit_buffer, int packet_header_len, const uint8_t *packet_data, int packet_length, int cardnum) {
	//add header outside of FEC
	wifi_packet_header_t *wph = (wifi_packet_header_t*)(packet_transmit_buffer + packet_header_len);
	wph->packet_number = seq_nr;
	...
	if (cardnum == 1) {
		...
		if (r1 != plen) {
			pcap_perror(ppcap1, "Trouble injecting packet on card 1");
			exit(1);
		}
	} else {
		int r2 = pcap_inject(ppcap2, packet_transmit_buffer, plen);
		if (r2 != plen) {
			pcap_perror(ppcap2, "Trouble injecting packet on card 2");
			exit(1);
		}
	}
}
		...
		while(di < data_packets_per_block || fi < fec_packets_per_block) {
			if (counter % 2) {
				if(di < data_packets_per_block) {
					pb_transmit_packet(ppcap1, ppcap2, seq_nr_tmp, packet_transmit_buffer, packet_header_len, data_blocks[di], packet_length, 0);
					seq_nr_tmp++;
					di++;
				}

				if(fi < fec_packets_per_block) {
					pb_transmit_packet(ppcap1, ppcap2, seq_nr_tmp, packet_transmit_buffer, packet_header_len, fec_blocks[fi], packet_length, 0);
					seq_nr_tmp++;
					fi++;
				}	
			} 
				...
				if(di < data_packets_per_block) {
					pb_transmit_packet(ppcap1, ppcap2, seq_nr_tmp, packet_transmit_buffer, packet_header_len, data_blocks[di], packet_length, 1);
					seq_nr_tmp++;
					di++;
				}

				if(fi < fec_packets_per_block) {
					pb_transmit_packet(ppcap1, ppcap2, seq_nr_tmp, packet_transmit_buffer, packet_header_len, fec_blocks[fi], packet_length, 1);
					seq_nr_tmp++;
					fi++;
				}	
			}
...
int main(int argc, char *argv[]) {
	...
	printf("tx_alternate Raw data transmitter (c) 2015 befinitiv. Dirty mods by Rodizio.  GPL2\n");
	...
	int x = optind;
	ppcap1 = pcap_open_live(argv[x], 800, 1, 20, szErrbuf1);
	if (ppcap1 == NULL) {
		fprintf(stderr, "Unable to open interface %s in pcap: %s\n", argv[x],szErrbuf1);
		return (1);
	}
	++x;
	ppcap2 = pcap_open_live(argv[x], 800, 1, 20, szErrbuf2);
	if (ppcap2 == NULL) {
		fprintf(stderr, "Unable to open interface %s in pcap: %s\n", argv[x],szErrbuf2);
		return (1);
	}
